/*
 -------------------------------------------------------------------------
  ...
  Descriptions    : 로그와 터미널로 프린트를 출력

  History         : 05/08/10       Initial Coded
 -------------------------------------------------------------------------
*/
...
/* global variables */
unsigned int    glTaskTraceMask = FMS_CLB | FMS_DBM | FMS_HST | FMS_VMC | FMS_BPT | FMS_PQM;
...
{
    if (mask == 0) {
        glTaskTraceMask = FMS_CLB | FMS_DBM | FMS_HST | FMS_VMC | FMS_BPT | FMS_PQM;
    } else {
        ...
        if (mask & FMS_CLB) {
            ...
            gMask = FMS_DBM;
        } else if (mask & FMS_HST) {
            sprintf(gIdent, "%s", "[ HST ] ");
            gMask = FMS_HST;
        } else if (mask & FMS_VMC) {
            sprintf(gIdent, "%s", "[ VMC ] ");
            gMask = FMS_VMC;
        } else if (mask & FMS_BPT) {
            sprintf(gIdent, "%s", "[ BPT ] ");
            gMask = FMS_BPT;
        } else if (mask & FMS_PQM) {
            sprintf(gIdent, "%s", "[ PQM ] ");
            gMask = FMS_PQM;
        } else {
    ...
    if ((gbDebugLevel >= (msgLevel & TRC_LEVEL_MASK)) && (TRACE_MASK & msgLevel & glTaskTraceMask)) {
...

        {
            ...
            if (((gbTrace & DBG_TERMINAL) && lastc == '\n') && !(msgLevel & ROW_PRINT)) {
                ...
                if (msgLevel & FMS_HST)                     printf("[ HST ] ");
                if (msgLevel & FMS_VMC)                     printf("[ VMC ] ");
                if (msgLevel & FMS_BPT)                     printf("[ BPT ] ");
                if (msgLevel & FMS_PQM)                     printf("[ PQM ] ");
                ...
                if (lastc_file == '\n') {
                    if (logfp != NULL) {
                        ...
                        if (!(msgLevel & ROW_PRINT)) {
                            ...
                            if (msgLevel & FMS_HST)                         sprintf(stackstr, "%s", "[ HST ] ");
                            if (msgLevel & FMS_VMC)                         sprintf(stackstr, "%s", "[ VMC ] ");
                            if (msgLevel & FMS_BPT)                         sprintf(stackstr, "%s", "[ BPT ] ");
                            if (msgLevel & FMS_PQM)                         sprintf(stackstr, "%s", "[ PQM ] ");
